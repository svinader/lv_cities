...
#Atlasīt tikai nosaukumu un novadu kolonnas 
SELECT title, region
FROM cities;

#Atlasīt pilsētas kuras ir vecākas par 100 gadiem
SELECT * FROM cities
WHERE year < 1924;

#Atlasīt pilsētas ar mazāk vai vienādas ar 3501 iedzīvotāju skaitu
SELECT * FROM cities
WHERE population <= 3501;

#Atlasīt top 10 lielākas pilsētas pēc iedzīvotāju skaita
SELECT * FROM cities
ORDER BY population DESC
LIMIT 10;

#Saskaitīt kopējo latvijas pilsētu skaitu
SELECT COUNT(title)
FROM cities;

#Atlasīt unikālus novada nosaukumus
SELECT DISTINCT region FROM cities;

#Dzēst ierakstu ar pilsētu “Durbe”
DELETE FROM cities WHERE title='Durbe';

SELECT * FROM cities;

#Atlasīt kopēju iedzīvotāji skaitu pēc novadiem
